...
function Articles() {
  ...
  const [tab, setTab] = useState("my"); // "my" | "published"
      ...
      {tab === "published" && (
        <div className="art-list">
          {publicArticles.length === 0 ? (
            <p className="art-empty">No published blogs or articles yet.</p>
          ) : (
            publicArticles.map((article) => renderArticleCard(article, userId && article.userId === userId))
          )}
        </div>
      )}
    </div>
